...
export function getEdicoesPorSlide(result, discovery, config = PRICING_DEFAULTS) {
  ...
  if (ativo('one_platform')) {
    ...
    // dois. Slide 14 entra só por 'one_platform' (CANAL_SLIDES), então esse tile precisa de
    // edição incondicional — senão fica com "R$ XX,XX" quando o cliente não tem email/portal.
    ...
    add(14, [{ shape: 'Google Shape;659;p55', type: 'run', run: 0, text: precoEmailMassivoPortal > 0 ? formatMoney(precoEmailMassivoPortal) : '' }])
  }
  ...
  // entra na proposta se QUALQUER UM dos 6 canais estiver ativo (ver CANAL_SLIDES) — então cada
  // tile precisa de edição própria sempre, senão o placeholder "R$ XX,XX" do template vaza pros
  // canais que não foram selecionados (mesma regra já aplicada nos slides 20/23).
  const precoEmailResumo = ativo('email') ? result.precos.email
    ...
    : null
  add(25, [{ shape: 'Google Shape;1012;p66', type: 'run', run: 0, text: precoEmailResumo !== null ? formatMoneyPreciso(precoEmailResumo) : '' }])
  ...
  const precoRcsResumo = ativo('rcs') ? result.precos.rcs
    ...
    : null
  add(25, [{ shape: 'Google Shape;1016;p66', type: 'run', run: 0, text: precoRcsResumo !== null ? formatMoneyPreciso(precoRcsResumo) : '' }])

  const smsAtivo = ['sms_texto', 'sms_fast_otp', 'sms_rota_exclusiva', 'sms_sender_id', 'sms_flash'].some(ativo)
  add(25, [{ shape: 'Google Shape;1021;p66', type: 'run', run: 0, text: smsAtivo ? formatMoneyPreciso(result.precos.sms) : '' }])
  ...
  add(25, [{ shape: 'Google Shape;1027;p66', type: 'run', run: 0, text: ativo('cartas_fisico') ? formatMoney(config.cartasFisico.fac_simples.A4['1x1']) : '' }])

  add(25, [{ shape: 'Google Shape;1031;p66', type: 'run', run: 0, text: ativo('google_meta_ads') ? formatMoneyPreciso(result.precos.google_meta_ads) : '' }])
  ...
  add(25, [{ shape: 'Google Shape;1035;p66', type: 'run', run: 0, text: precoWhatsResumo !== null ? formatMoneyPreciso(precoWhatsResumo) : '' }])
  ...
  // modelados ainda, mas o slide só entra via 'carne' (CANAL_SLIDES) — precisam ficar em
  // branco sempre, senão sobra "R$ XX,XX" de um produto que nem foi cotado.
  if (ativo('carne')) {
    add(26, [
      { shape: 'Google Shape;1047;p67', type: 'run', run: 0, text: formatMoneyPreciso(result.precos.carne) },
      { shape: 'Google Shape;1051;p67', type: 'run', run: 0, text: '' },
      { shape: 'Google Shape;1055;p67', type: 'run', run: 0, text: '' },
    ])
  }
  ...
  // estiver ativo — cada tile precisa de edição incondicional (mesma regra do slide 25).
  // "Contact Score" não tem produto próprio modelado — fica sempre em branco.
  ...
  if (precoSmartContactResumo !== null || ativo('valida_mais') || precoEnriquecimentoResumo !== null) {
    add(30, [
      { shape: 'Google Shape;1106;p71', type: 'run', run: 0, text: precoSmartContactResumo !== null ? formatMoneyPreciso(precoSmartContactResumo) : '' },
      { shape: 'Google Shape;1110;p71', type: 'run', run: 0, text: ativo('valida_mais') ? formatMoneyPreciso(result.precos.valida_mais) : '' },
      { shape: 'Google Shape;1114;p71', type: 'run', run: 0, text: precoEnriquecimentoResumo !== null ? formatMoneyPreciso(precoEnriquecimentoResumo) : '' },
      { shape: 'Google Shape;1118;p71', type: 'run', run: 0, text: '' },
    ])
  }
