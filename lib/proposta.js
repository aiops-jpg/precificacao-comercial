...
function computeCampos(result, discovery, config) {
  ...
  const campos = []
  // `shape` é o shape "canônico" usado pra posicionar o overlay no editor visual — pra campos com
  // dedup entre 2 slides (ex. preco_sms), é o do local mais limpo pra virar um input (o card
  // isolado, não o trecho embutido numa frase). Os 3 campos de Identificação não têm shape (não
  // têm mapeamento 1:1 com uma única caixa de texto) e ficam de fora do overlay.
  const add = (id, group, label, value, shape) => campos.push({ id, group, label, value, shape })
  const flags = {}
  ...
  if (ativo('one_platform')) {
    add('one_setup', 'ONE Platform', 'Setup ONE Platform (R$)', formatCompacto(setupONE(result.plano_one, config)), 'Google Shape;647;p54')
  ...
  if (ativo('voicebot')) {
    add('voicebot_setup', 'Voicebot', 'Setup Voicebot (R$)', formatCompacto(config.setup.voicebot), 'Google Shape;702;p57')
    add('voicebot_valor_por_robo', 'Voicebot', 'Valor por Robô Extra', formatMoney(2000), 'Google Shape;688;p57')
    add('voicebot_limite_conversas', 'Voicebot', 'Limite de Conversas por Robô', '200', 'Google Shape;717;p57')
  }

  if (ativo('chatbot')) {
    add('chatbot_setup', 'Chatbot', 'Setup Chatbot (R$)', formatCompacto(config.setup.chatbot), 'Google Shape;736;p58')
    add('chatbot_franquia', 'Chatbot', 'Franquia de Negociações', '4.000', 'Google Shape;731;p58')
    add('chatbot_preco_negociacao', 'Chatbot', 'Preço por Negociação Extra', '0,40', 'Google Shape;752;p58')
  }

  flags.smsAtivoSlide20 = ['sms_texto', 'sms_fast_otp', 'sms_rota_exclusiva', 'sms_sender_id', 'sms_flash'].some(ativo)
  if (flags.smsAtivoSlide20) add('preco_sms', 'Detalhamento de Canais', 'SMS — preço unitário', formatMoneyPreciso(result.precos.sms), 'Google Shape;792;p61')
  ...
  if (flags.precoEnriquecimentoAtivo !== null) {
    add('preco_enriquecimento_ativo', 'Detalhamento de Canais', ativo('enriquecimento_premium') ? 'Enriquecimento Premium — preço' : 'Enriquecimento — preço', formatMoneyPreciso(flags.precoEnriquecimentoAtivo), 'Google Shape;802;p61')
  }

  flags.validaAtivoSlide20 = ativo('valida_mais')
  if (flags.validaAtivoSlide20) add('preco_valida_mais', 'Detalhamento de Canais', 'Valida+ — preço unitário', formatMoneyPreciso(result.precos.valida_mais), 'Google Shape;811;p61')
  ...
  if (ativo('rcs') || ativo('rcs_basico') || ativo('rcs_simples')) {
    if (ativo('rcs_basico')) add('rcs_basico_detalhe', 'RCS', 'RCS Básico — preço (detalhamento)', formatNumeroPreciso(result.precos.rcs_basico), 'Google Shape;923;p64')
    if (ativo('rcs_simples')) add('rcs_simples_detalhe', 'RCS', 'RCS Simples — preço (detalhamento)', formatNumeroPreciso(result.precos.rcs_simples), 'Google Shape;930;p64')
  }

  if (ativo('whats_ativo') || ativo('whats_receptivo')) {
    add('whatsapp_setup', 'WhatsApp', 'Setup WhatsApp (R$)', formatCompacto(config.setup.whatsapp), 'Google Shape;833;p62')
    add('whatsapp_preco_mt', 'WhatsApp', 'Preço MT (mensagem enviada pela empresa)', '0,18', 'Google Shape;843;p62')
    add('whatsapp_preco_mo', 'WhatsApp', 'Preço MO (sessão iniciada pelo cliente)', '0,18', 'Google Shape;844;p62')
  }

  if (ativo('portal_negociacao')) {
    add('portal_setup', 'Portal de Negociação', 'Setup Portal de Negociação', formatMoney(config.setup.portal_negociacao), 'Google Shape;1081;p69')
    add('portal_limite_criacao', 'Portal de Negociação', 'Limite de Criação (linhas)', '15.000', 'Google Shape;1084;p69')
  }

  if (ativo('landing_page_link')) {
    add('landing_setup', 'Landing Page', 'Setup Landing Page', formatMoney(config.setup.landing_page_link), 'Google Shape;1094;p70')
    add('landing_valor_por_link', 'Landing Page', 'Valor por Link (slide dedicado)', '0,02', 'Google Shape;1097;p70')
  }
