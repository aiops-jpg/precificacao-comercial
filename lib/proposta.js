...
function computeCampos(result, discovery, config) {
  ...
  const campos = []
  const add = (id, group, label, value) => campos.push({ id, group, label, value })
  const flags = {}
  ...
  if (ativo('one_platform')) {
    add('one_setup', 'ONE Platform', 'Setup ONE Platform (R$)', formatCompacto(setupONE(result.plano_one, config)))
    add('one_total_mensal', 'ONE Platform', 'Recorrente Mensal ONE Platform', formatMoney(result.plano_one.total_mensal))
    ...
    if (flags.emailOuPortalAtivo) {
      ...
      add('one_email_portal_mensal', 'ONE Platform', 'E-mail Massivo + Portal (mensal)', formatMoney(precoEmailMassivoPortal))
    }
  }

  if (ativo('voicebot')) {
    add('voicebot_setup', 'Voicebot', 'Setup Voicebot (R$)', formatCompacto(config.setup.voicebot))
    add('voicebot_valor_por_robo', 'Voicebot', 'Valor por Robô Extra', formatMoney(2000))
    add('voicebot_limite_conversas', 'Voicebot', 'Limite de Conversas por Robô', '200')
  }

  if (ativo('chatbot')) {
    add('chatbot_setup', 'Chatbot', 'Setup Chatbot (R$)', formatCompacto(config.setup.chatbot))
    add('chatbot_franquia', 'Chatbot', 'Franquia de Negociações', '4.000')
    add('chatbot_preco_negociacao', 'Chatbot', 'Preço por Negociação Extra', '0,40')
  }

  flags.smsAtivoSlide20 = ['sms_texto', 'sms_fast_otp', 'sms_rota_exclusiva', 'sms_sender_id', 'sms_flash'].some(ativo)
  if (flags.smsAtivoSlide20) add('preco_sms', 'Detalhamento de Canais', 'SMS — preço unitário', formatMoneyPreciso(result.precos.sms))

  flags.emailSimplesAtivo = ativo('email')
  flags.emailRegistradoAtivo = ativo('email_registrado')
  if (flags.emailSimplesAtivo) add('preco_email', 'Detalhamento de Canais', 'E-mail Simples — preço unitário', formatMoneyPreciso(result.precos.email))
  if (flags.emailRegistradoAtivo) add('preco_email_registrado', 'Detalhamento de Canais', 'E-mail Registrado — preço unitário', formatMoneyPreciso(result.precos.email_registrado))

  flags.documentoDigitalAtivo = ativo('documento_digital')
  flags.cartorioAtivo = ativo('cartorio_documento')
  if (flags.documentoDigitalAtivo) add('preco_documento_digital', 'Detalhamento de Canais', 'Documento Digital — preço unitário', formatMoneyPreciso(result.precos.documento_digital))
  if (flags.cartorioAtivo) add('preco_cartorio_documento', 'Detalhamento de Canais', 'Cartório — preço unitário', formatMoneyPreciso(result.precos.cartorio_documento))
  ...
  if (flags.precoEnriquecimentoAtivo !== null) {
    add('preco_enriquecimento_ativo', 'Detalhamento de Canais', ativo('enriquecimento_premium') ? 'Enriquecimento Premium — preço' : 'Enriquecimento — preço', formatMoneyPreciso(flags.precoEnriquecimentoAtivo))
  }

  flags.validaAtivoSlide20 = ativo('valida_mais')
  if (flags.validaAtivoSlide20) add('preco_valida_mais', 'Detalhamento de Canais', 'Valida+ — preço unitário', formatMoneyPreciso(result.precos.valida_mais))

  flags.landingAtivoSlide20 = ativo('landing_page_link')
  if (flags.landingAtivoSlide20) add('preco_landing_page_link', 'Detalhamento de Canais', 'Landing Page — preço por link (detalhamento)', formatMoneyPreciso(result.precos.landing_page_link))
  ...
  if (ativo('rcs') || ativo('rcs_basico') || ativo('rcs_simples')) {
    if (ativo('rcs_basico')) add('rcs_basico_detalhe', 'RCS', 'RCS Básico — preço (detalhamento)', formatNumeroPreciso(result.precos.rcs_basico))
    if (ativo('rcs_simples')) add('rcs_simples_detalhe', 'RCS', 'RCS Simples — preço (detalhamento)', formatNumeroPreciso(result.precos.rcs_simples))
  }

  if (ativo('whats_ativo') || ativo('whats_receptivo')) {
    add('whatsapp_setup', 'WhatsApp', 'Setup WhatsApp (R$)', formatCompacto(config.setup.whatsapp))
    add('whatsapp_preco_mt', 'WhatsApp', 'Preço MT (mensagem enviada pela empresa)', '0,18')
    add('whatsapp_preco_mo', 'WhatsApp', 'Preço MO (sessão iniciada pelo cliente)', '0,18')
  }

  if (ativo('portal_negociacao')) {
    add('portal_setup', 'Portal de Negociação', 'Setup Portal de Negociação', formatMoney(config.setup.portal_negociacao))
    add('portal_limite_criacao', 'Portal de Negociação', 'Limite de Criação (linhas)', '15.000')
  }

  if (ativo('landing_page_link')) {
    add('landing_setup', 'Landing Page', 'Setup Landing Page', formatMoney(config.setup.landing_page_link))
    add('landing_valor_por_link', 'Landing Page', 'Valor por Link (slide dedicado)', '0,02')
  }

  if (ativo('carne')) {
    add('preco_carne', 'Carnê', 'Carnê — preço unitário', formatMoneyPreciso(result.precos.carne))
  }
  ...
  flags.precoEmailResumo = flags.emailSimplesAtivo ? result.precos.email
    ...
    : null
  if (flags.precoEmailResumo !== null) add('preco_email_resumo', 'Card-Resumo', 'E-mail (card-resumo) — preço', formatMoneyPreciso(flags.precoEmailResumo))
  ...
  flags.precoRcsResumo = ativo('rcs') ? result.precos.rcs
    ...
    : null
  if (flags.precoRcsResumo !== null) add('preco_rcs_resumo', 'Card-Resumo', 'RCS (card-resumo) — preço', formatMoneyPreciso(flags.precoRcsResumo))

  if (ativo('cartas_fisico')) add('preco_cartas_fisico_resumo', 'Card-Resumo', 'Cartas/Físico (card-resumo) — a partir de', formatMoney(config.cartasFisico.fac_simples.A4['1x1']))

  if (ativo('google_meta_ads')) add('preco_google_meta_ads', 'Card-Resumo', 'Google / Meta Ads — preço', formatMoneyPreciso(result.precos.google_meta_ads))
  ...
  if (flags.precoWhatsResumo !== null) add('preco_whats_resumo', 'Card-Resumo', 'WhatsApp (card-resumo) — preço', formatMoneyPreciso(flags.precoWhatsResumo))
  ...
  flags.precoSmartContactResumo = ativo('smart_contact_phone_score') ? result.precos.smart_contact_phone_score
    ...
    : null
  if (flags.precoSmartContactResumo !== null) add('preco_smart_contact_resumo', 'Card-Resumo', 'Smart Contact (card-resumo) — preço', formatMoneyPreciso(flags.precoSmartContactResumo))
...
export function getEdicoesPorSlide(result, discovery, config = PRICING_DEFAULTS, overrides = {}) {
  ...
  add(20, cardOuRemove(smsAtivoSlide20, 'Google Shape;792;p61', val('preco_sms'),
  ...
  // estiverem inativos.
  add(20, cardOuRemove(emailSimplesAtivo, 'Google Shape;788;p61', val('preco_email'), ['Google Shape;787;p61']))
  add(20, cardOuRemove(emailRegistradoAtivo, 'Google Shape;790;p61', val('preco_email_registrado'), ['Google Shape;789;p61']))
  ...
  add(20, cardOuRemove(documentoDigitalAtivo, 'Google Shape;807;p61', val('preco_documento_digital'), ['Google Shape;804;p61']))
  add(20, cardOuRemove(cartorioAtivo, 'Google Shape;808;p61', val('preco_cartorio_documento'), ['Google Shape;805;p61']))
  if (!documentoDigitalAtivo && !cartorioAtivo) {
    ...
    add(20, [{ shape: 'Google Shape;809;p61', type: 'remove' }])
  }
  ...
  add(20, cardOuRemove(enriquecimentoAtivoSlide20, 'Google Shape;802;p61', val('preco_enriquecimento_ativo'), ['Google Shape;801;p61']))
  add(20, cardOuRemove(validaAtivoSlide20, 'Google Shape;811;p61', val('preco_valida_mais'), ['Google Shape;810;p61']))
  ...
  add(20, cardOuRemove(landingAtivoSlide20, 'Google Shape;818;p61', val('preco_landing_page_link'),
  ...
  add(25, cardOuRemove(flags.precoEmailResumo !== null, 'Google Shape;1012;p66', val('preco_email_resumo'),
    ['Google Shape;1011;p66', 'Google Shape;1013;p66', 'Google Shape;1014;p66', 'Google Shape;1019;p66']))

  add(25, cardOuRemove(flags.precoRcsResumo !== null, 'Google Shape;1016;p66', val('preco_rcs_resumo'),
    ['Google Shape;1015;p66', 'Google Shape;1017;p66', 'Google Shape;1018;p66', 'Google Shape;1025;p66']))

  add(25, cardOuRemove(smsAtivoSlide20, 'Google Shape;1021;p66', val('preco_sms'),
  ...
  add(25, cardOuRemove(ativo('cartas_fisico'), 'Google Shape;1027;p66', val('preco_cartas_fisico_resumo'),
    ['Google Shape;1026;p66', 'Google Shape;1028;p66', 'Google Shape;1029;p66', 'Google Shape;1039;p66']))

  add(25, cardOuRemove(ativo('google_meta_ads'), 'Google Shape;1031;p66', val('preco_google_meta_ads'),
    ['Google Shape;1030;p66', 'Google Shape;1032;p66', 'Google Shape;1033;p66', 'Google Shape;1040;p66']))

  add(25, cardOuRemove(flags.precoWhatsResumo !== null, 'Google Shape;1035;p66', val('preco_whats_resumo'),
  ...
  if (flags.precoSmartContactResumo !== null || validaAtivoSlide20 || enriquecimentoAtivoSlide20) {
    add(30, cardOuRemove(flags.precoSmartContactResumo !== null, 'Google Shape;1106;p71', val('preco_smart_contact_resumo'),
      ['Google Shape;1105;p71', 'Google Shape;1107;p71', 'Google Shape;1108;p71', 'Google Shape;1122;p71']))
    add(30, cardOuRemove(validaAtivoSlide20, 'Google Shape;1110;p71', val('preco_valida_mais'),
      ['Google Shape;1109;p71', 'Google Shape;1111;p71', 'Google Shape;1112;p71', 'Google Shape;1123;p71']))
    add(30, cardOuRemove(enriquecimentoAtivoSlide20, 'Google Shape;1114;p71', val('preco_enriquecimento_ativo'),
