...
function computeCampos(result, discovery, config) {
  ...
  if (ativo('whats_ativo') || ativo('whats_receptivo')) {
    ...
    add('whatsapp_preco_conversa', 'WhatsApp', 'Preço Conversação (janela de atendimento 24h)', '0,025', 'WhatsAppConversacao_preco', 1)
  }
...
export function getEdicoesPorSlide(result, discovery, config = PRICING_DEFAULTS, overrides = {}) {
  ...
  if (ativo('whats_ativo') || ativo('whats_receptivo')) {
    add(21, [
      ...
      { shape: 'WhatsAppConversacao_preco', type: 'run', run: 1, text: val('whatsapp_preco_conversa') },
    ])
  }
