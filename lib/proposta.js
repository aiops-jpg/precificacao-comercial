...
export function getEdicoesPorSlide(result, discovery, config = PRICING_DEFAULTS) {
  ...
  if (ativo('cartas_fisico')) add(25, [{ shape: 'Google Shape;1027;p66', type: 'run', run: 0, text: formatMoneyPreciso(config.cartasFisico.fac_simples.A4['1x1']) }])
      ...
      for (const formato of ['A4', 'A3']) {
        for (const combo of ['1x1', '2x1', '4x1', '4x4']) {
          edicoesCartas.push({
            ...
            type: 'run',
            run: 0,
            text: formatMoneyPreciso(tipo[formato][combo]),
          })
        }
      }
    }
