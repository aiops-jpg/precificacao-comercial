...
export function ConfigProvider({ children }) {
  ...
  const carregar = useCallback(async () => {
    ...
    try {
      ...
      // Faz merge contra PRICING_DEFAULTS mesmo pra base GERAL — uma linha salva no banco
      // ANTES de um campo novo ser criado (ex: setup.horas_incluidas) fica sem essa chave,
      // e sem isso o app mostraria o campo em branco pra todo mundo até um admin republicar.
      if (resp.ok) base = mergeComPadrao(PRICING_DEFAULTS, await resp.json())
    } catch {
